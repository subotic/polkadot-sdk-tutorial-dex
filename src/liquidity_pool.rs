...
use frame_support::{ensure, RuntimeDebugNoBound as RuntimeDebug};
...
impl<T: Config> LiquidityPool<T> {
	// Function to mint liquitity tokens and update reserves
	pub fn mint(
		&mut self,
		...
		self.reserves.0 =
			self.reserves.0.checked_add(&amounts.0).ok_or(Error::<T>::ReserveOverflow)?;
		self.reserves.1 =
			self.reserves.1.checked_add(&amounts.1).ok_or(Error::<T>::ReserveOverflow)?;
	...
	pub fn swap(
		...
		if self.assets.0 == asset_in {
			self.reserves.0 =
				self.reserves.0.checked_add(&amount_in).ok_or(Error::<T>::ReserveOverflow)?;
			self.reserves.1 = self
				.reserves
				.1
			...
			self.reserves.0 = self
				...
				.ok_or(Error::<T>::InsufficientReserves)?;
			self.reserves.1 =
				self.reserves.1.checked_add(&amount_in).ok_or(Error::<T>::ReserveOverflow)?;
		}
	...
	fn get_amount_out(
		...
		let swapp_fee = Permill::from_perthousand(3);

		// Calculate the input amount with the swap fee (0.3%) by multiplying by 997 (99.7%)
		let amount_in_after_fee = amount_in
			.checked_sub(&swapp_fee.mul_floor(amount_in))
