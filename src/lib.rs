...
pub mod pallet {
	...
	/// Storage map for storing mappinmg of Liquidity token to asset pair
	#[pallet::storage]
	pub type LiquidityToken<T: Config> =
	...
	pub enum Event<T: Config> {
		...
		LiquidityMinted(AccountIdOf<T>, (AssetIdOf<T>, AssetIdOf<T>), AssetBalanceOf<T>),
		...
		LiquidityBurned(AccountIdOf<T>, (AssetIdOf<T>, AssetIdOf<T>), AssetBalanceOf<T>),
		...
		Swapped(AccountIdOf<T>, AssetIdOf<T>, AssetBalanceOf<T>, AssetIdOf<T>, AssetBalanceOf<T>),
	}
	...
	impl<T: Config> Pallet<T> {
		...
		// Dispatchable call to mint liquidity tokens
		#[pallet::call_index(1)]
		#[pallet::weight(Weight::default())]
	...
	impl<T: Config> Pallet<T> {
		...
		fn geometric_mean(
			...
			let sqrt_product =
				(amount_a.checked_mul(&amount_b).ok_or(Error::<T>::ArithmeticOverflow)?)
					.integer_sqrt();
			Ok(sqrt_product)
		}
